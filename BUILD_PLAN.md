...
| Extension UI | **Angular** | Popup, block screen, options page. |
| Backend | **.NET (ASP.NET Core)**, localhost | Always-on, lightweight. Storage + decision cascade + reports + Python lifecycle. |
| Storage | **SQLite + EF Core** | Single-file local DB. |
...
3. **Schema already has the ML columns** (nullable): `FocusSession.intentEmbedding` (blob), `Label.features` (json). No migration pain when M2 starts populating them.
...
- **FocusSession** — id, startedAt, endedAt?, declaredIntent (text), intentEmbedding (blob, later).
...
- **Label** — id, ts, url, title, decision (allow/block), source (good-call/bad-call/bounce/active-query), features (json, later). *(feeds active learning)*
...
```
EnhancedBlocker/
  IDEA.md  ARCHITECTURE.md  BUILD_PLAN.md
  backend/        # .NET solution (ASP.NET Core + EF Core + SQLite)
  extension/      # Angular MV3 extension
...
**Backend (.NET)**
- [ ] ASP.NET Core project; SQLite + EF Core; entities above; migrations.
